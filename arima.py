...
def calculate_wnba_features(df, today, home_teams, away_teams):
    ...
    for i in superstars:
        ...
        try:
            ...
            gamelog['FGM'] = gamelog['FGM'].astype(float)
    ...
    dataset = []
    headers = ["FGM T", "FGA T", "Home", "Minutes Diff", "Rest Days", "Recent T"]
    ...
    for index, row in df.iterrows():
        try:
            ...
            sample_mean_fg_pct = gamelog.loc[row_index + 1 :, "eFG%"].mean()
            # rolling_avg_fg_pct = gamelog.loc[row_index + 1 : row_index + 3, "FGA"].mean() 
            fg_games_list = gamelog.loc[row_index + 1 : row_index + num_games, "eFG%"].tolist()
            difference_fgm = calculate_t_statistic(fg_games_list, np.mean(fg_games_list), sample_mean_fg_pct)
            print(difference_fgm)

            sample_mean_fg_pct = gamelog.loc[row_index + 1 :, "FGA"].mean()
            # rolling_avg_fg_pct = gamelog.loc[row_index + 1 : row_index + 3, "FGA"].mean() 
            fg_games_list = gamelog.loc[row_index + 1 : row_index + num_games, "FGA"].tolist()
            difference_fga = calculate_t_statistic(fg_games_list, np.mean(fg_games_list), sample_mean_fg_pct)
            ...
            last_5_minutes = gamelog.loc[row_index + 1 : row_index + num_games, "MIN"]
            ...
            last_5_points = gamelog.loc[row_index + 1 : row_index + num_games, "PTS"]
            ...
            if not today and difference_fga == 0 and difference_mins == 0:
                continue
            ...
            if not today:
                OU_result = gamelog.loc[row_index, 'PTS'] - np.median(last_5_points)
                # ["FGM T", "FGA T", "Home", "Minutes Diff", "Rest Days", "Recent T"]
                dataset.append([difference_fgm, difference_fga, home, difference_mins, rest_days, recent_t_statistic, OU_result])
            else:
    ...
    new_df['FGA T'] = pd.to_numeric(new_df['FGA T'], errors='coerce')
...
if __name__ == "__main__":
    ...
    new_df.to_csv("data/wnba_correlations.csv", index=False)